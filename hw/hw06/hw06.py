...
class VendingMachine:
    ...
    def vend(self):
        if self.stocks == 0:
            ...
            self.stocks -= 1
            if self.funds > 0:
                response = "Here is your {} and ${} change.".format(self.item,self.funds)
                self.funds = 0
                return response
            else:
    ...
    def add_funds(self,fund):
        self.funds += fund
        if self.stocks == 0:
            response = 'Inventory empty. Restocking required. Here is your ${}.'.format(self.funds)
            self.funds = 0
            return response
        else:
    ...
    def restock(self,stock):
        self.stocks += stock
        return 'Current {} stock: {}'.format(self.item,self.stocks)
...
class Coin:
    ...
    def worth(self):
        ...
        if isinstance(self,Nickel):
            if extra > 0:
                return Nickel.cents + extra
            else:
                return Nickel.cents
        elif isinstance(self,Dime):
            if extra > 0:
                return Dime.cents + extra
            else:
                return Dime.cents
...
def is_bst(t):
    ...
    if  t.is_leaf():
        ...
        if t.label > t.branches[0].label: #left
...
def store_digits(n):
    ...
    tail = Link(n % 10,Link.empty)
    return helper(tail,n//10)
